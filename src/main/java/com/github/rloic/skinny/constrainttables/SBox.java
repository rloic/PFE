package com.github.rloic.skinny.constrainttables;

import org.chocosolver.solver.constraints.extension.Tuples;

import java.util.HashMap;
import java.util.Map;

public class SBox {

    public static Tuples skinny128 = createRelationSbox();

    private static Tuples createRelationSbox() {

        int[] SBOX_VALUES = new int[]{
                
        };

        int[][] trans = new int[256][127];
        int[][] probas = new int[256][256];
        int[] ctrans = new int[256];
        Tuples tuples = new Tuples(true);
        Map<Integer, Integer> map = new HashMap<>();
        map.put(2,70);
        map.put(4,60);
        map.put(6,54);
        map.put(8,50);
        map.put(12,44);
        map.put(16,40);
        map.put(20,37);
        map.put(24,34);
        map.put(28,32);
        map.put(32,30);
        map.put(40,27);
        map.put(48,24);
        map.put(64,20);

        for (int i = 0; i < 256; i++) {
            ctrans[i] = 0;
            for (int j = 0; j < 256; j++) {
                probas[i][j] = 0;
                if (j < 127)
                    trans[i][j] = 0;
            }
        }
        int[] sboxes = SBOX_VALUES;
        for (int i = 0; i < 256; i++) {
            for (int j = 0; j < 256; j++) {
                probas[i][(SBOX_VALUES[j] ^ SBOX_VALUES[j ^ i])]++;
                if (probas[i][(SBOX_VALUES[j] ^ SBOX_VALUES[j ^ i])] == 1) {
                    trans[i][ctrans[i]++] = SBOX_VALUES[j] ^ SBOX_VALUES[i ^ j];
                }
            }
        }

        tuples.add(0, 0, 0);
        int p;
        for (int i = 1; i < 256; i++) {
            for (int j = 0; j < ctrans[i]; j++) {
                p = probas[i][trans[i][j]];
                tuples.add(i, trans[i][j], map.get(p));
            }
        }
        return tuples;
    }

}
