package com.github.rloic.aes.advanced.stepround;
...
public class AESAdvancedRoundNoTransit {
    ...
    public AESAdvancedRoundNoTransit(
            int r,
            int objStep1,
            KeyBits keyBits
    ) {
        this.m = new Model("AES Advanced Round[No Transit](r=" + r + ", objStep=" + objStep1 + ")");
        this.r = r;
        this.KEY_BITS = keyBits;

        ΔX = buildΔX(r, 4, 4);
        ΔY = c4(ΔX); // C'4 shift nbEquations
        ΔZ = c6(ΔY);
        ΔK = buildΔK(r, 4, 5);
        ...
        for (int i = 0; i < r; i++) {
            nbActives[i] = m.intVar("nbActives[" + i + "]", 0, objStep1);
    ...
    private BoolVar[] asArray(List<BoolVar> collection) {
        BoolVar[] result = new BoolVar[collection.size()];
