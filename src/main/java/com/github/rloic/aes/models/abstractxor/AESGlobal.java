package com.github.rloic.aes.models.abstractxor;
...
public class AESGlobal {
    ...
    public AESGlobal(
        ...
        if (nbActives != null) {
            for (int i = 0; i < r; i++) {
                List<BoolVar> sBoxesAtRound = new ArrayList<>(4 * 5);
                for (int j = 0; j < 4; j++) {
                    for (int k = 0; k < 4; k++) {
                        sBoxesAtRound.add(ΔX[i][j][k]);
                    }
                    if (keyBits.isSBRound(i)) {
                        sBoxesAtRound.add(ΔK[i][j][4]);
                    }
                }
                em.sum(asArray(sBoxesAtRound), "=", nbActives[i]);
            }
        }
