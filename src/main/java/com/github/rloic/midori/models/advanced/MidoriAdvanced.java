package com.github.rloic.midori.models.advanced;
...
public class MidoriAdvanced {
   ...
   public MidoriAdvanced(
      ...
      for (int i = 0; i < r - 1; i++) {
         for (int j = 0; j < 4; j++) {
            for (int k = 0; k < 4; k++) {
               m.sum(arrayOf(ΔZ[i][j][k], ΔK[j][k], ΔX[i + 1][j][k]), "!=", 1).post();
            }
         }
      }
      for (int i = 0; i < r - 1; i++) {
         ...
         ΔY[i][3][3] = ΔX[i][0][2];
         for (int k = 0; k < 4; k++) {
            m.sum(arrayOf(ΔY[i][1][k], ΔY[i][2][k], ΔY[i][3][k], ΔZ[i][0][k]), "!=", 1).post();
            m.sum(arrayOf(ΔY[i][0][k], ΔY[i][2][k], ΔY[i][3][k], ΔZ[i][1][k]), "!=", 1).post();
            m.sum(arrayOf(ΔY[i][0][k], ΔY[i][1][k], ΔY[i][3][k], ΔZ[i][2][k]), "!=", 1).post();
            m.sum(arrayOf(ΔY[i][0][k], ΔY[i][1][k], ΔY[i][2][k], ΔZ[i][3][k]), "!=", 1).post();
      ...
      for (int i1 = 0; i1 <= r - 2; i1++) {
         for (int j = 0; j <= 3; j++) {
            for (int k = 0; k <= 3; k++) {
               for (int i2 = 0; i2 < i1; i2++) {
                  m.sum(arrayOf(diffZ[i1][j][k][i2], ΔZ[i1][j][k], ΔZ[i2][j][k]), "!=", 1).post();
                  m.sum(arrayOf(diffY[i1][j][k][i2], ΔY[i1][j][k], ΔY[i2][j][k]), "!=", 1).post();
                  m.sum(arrayOf(diffZ[i1][j][k][i2], ΔX[i1 + 1][j][k], ΔX[i2 + 1][j][k]), "!=", 1).post();
                  for (int i3 = 0; i3 < i2; i3++) {
                     m.sum(arrayOf(diffY[i1][j][k][i2], diffY[i2][j][k][i3], diffY[i1][j][k][i3]), "!=", 1).post();
                     m.sum(arrayOf(diffZ[i1][j][k][i2], diffZ[i2][j][k][i3], diffZ[i1][j][k][i3]), "!=", 1).post();
                  }
               }
            }
         }
